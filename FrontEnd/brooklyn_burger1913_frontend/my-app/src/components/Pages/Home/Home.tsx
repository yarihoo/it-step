...
const Home = () => {
    ...
    const [dealProducts, setDealProducts] = useState<Array<IProductItem>>([]);
    ...
    useEffect(() => {
        ...
        axios.get<Array<IProductItem>>('https://localhost:7048/Deals').then(resp => {
            setDealProducts(resp.data);
        });
    }, []);

    const getDeal = (num: number) =>{
        if (!(num > dealProducts.length - 1))
        {
            let item: IProductItem = dealProducts[num];
            return (
                    <div id='main-frame'>
                        <div style={{position: "relative"}}>
                            <div onClick={() => MoveDeal(num, false)} style={{left: "30%"}} className='arrow-sign'>&lt;</div>
                            <div className='image-box'>
                                <img
                                src={item.image}
                                className='deal-item-image'></img>
                            </div>
                            <div onClick={() => MoveDeal(num, true)} style={{right: "30%"}} className='arrow-sign'>&gt;</div>
                        </div>
                        <div className='deal-name'>{item.name}</div>
                        <div className='deal-description'>{item.description}</div>
                        <div style={{textDecoration: "line-through"}} className='deal-price'>{item.price}$</div>
                        <div style={{color: "yellow"}} className='deal-price'>{item.salePrice}$</div>
                        <div className='main-frame-button'>Add To Bag</div>
                        <Link to="/menu" className='main-frame-button'>Get All Deals</Link>
                    </div>
            );
        }
        else{
                ...
                {category.subcategories.map((subcategory) => {
                  return (
                    <React.Fragment key={subcategory.id}>
                      ...
                      <div
                        style={{ width: "50%", margin: "1vw auto 3vw auto" }}
                        className="white-line"
                        key={`white-line-${subcategory.id}`}
                      ></div>
                        ...
                        <tbody>
                          {subcategory.products.slice(0, 3).map((product) => (
                            <tr key={product.id}>
                              ...
                              <td className="product-text">{GetPrice(product)}</td>
                              <td className="product-text">
                                <input
                                  id={`count${product.id}`}
                                  className="counter"
    ...
    function MoveDeal(position: number, direction: boolean){
        if (direction){
            if (position !== dealProducts.length - 1){
                setCurrentDealIndex(position + 1);
            }
            else{
            ...
            if (position !== 0){
                ...
                setCurrentDealIndex(dealProducts.length - 1);
            }
        }
    }
    return (
        <>
            <div id='deals-frame'>{getDeal(currentDealIndex)}</div>
            {Banner()}
            <div className='white-line'></div>
            {PopularItems(categories)}
